 ...
 * @param {Function} props.onNext - 次へボタン押下時のコールバック
 * @returns {JSX.Element}
 */
const RelationSettingTable = ({ accounts, onChange, onNext }) => {
  // リレーションタイプ別にアカウントをグループ化
  const { ppeAccounts, retainedEarningsAccounts, workingCapitalAccounts } =
    useMemo(() => {
      // PP&E（有形固定資産）関連のアカウント
      const ppeAccounts = accounts.filter(
        (account) =>
          account.relation && account.relation.type === RELATION_TYPES.PPE
      );
      ...
      const retainedEarningsAccounts = accounts.filter(
        (account) =>
          account.relation &&
          account.relation.type === RELATION_TYPES.RETAINED_EARNINGS
      );
      ...
      const workingCapitalAccounts = accounts.filter(
        (account) =>
          account.relation &&
          account.relation.type === RELATION_TYPES.WORKING_CAPITAL
      );

      return { ppeAccounts, retainedEarningsAccounts, workingCapitalAccounts };
    }, [accounts]);
  ...
  const assetAccounts = useMemo(() => {
    return ppeAccounts.filter(
      (account) =>
        account.relation &&
        account.relation.subType === RELATION_SUB_TYPES.ASSET
    );
  }, [ppeAccounts]);
  ...
  const investmentAccounts = useMemo(() => {
    return ppeAccounts.filter(
      (account) =>
        account.relation &&
        account.relation.subType === RELATION_SUB_TYPES.INVESTMENT
    );
  }, [ppeAccounts]);
  ...
  const depreciationAccounts = useMemo(() => {
    return ppeAccounts.filter(
      (account) =>
        account.relation &&
        account.relation.subType === RELATION_SUB_TYPES.DEPRECIATION
    );
  }, [ppeAccounts]);
  ...
  const retainedEarningsAssetAccounts = useMemo(() => {
    return retainedEarningsAccounts.filter(
      (account) =>
        account.relation &&
        account.relation.subType === RELATION_SUB_TYPES.RETAINED
    );
  ...
  const profitAccounts = useMemo(() => {
    return accounts.filter(
      (account) =>
        account.relation &&
        // account.relation.type === RELATION_TYPES.RETAINED_EARNINGS &&
        account.relation.subType === RELATION_SUB_TYPES.PROFIT
    );
  }, [accounts]);

  // 運転資本資産科目のオブジェクトリスト
  const workingCapitalAssetAccounts = useMemo(() => {
    return workingCapitalAccounts.filter(
      (account) =>
        account.relation &&
        account.relation.subType === RELATION_SUB_TYPES.WC_ASSET
    );
  }, [workingCapitalAccounts]);

  // 運転資本負債科目のオブジェクトリスト
  const workingCapitalLiabilityAccounts = useMemo(() => {
    return workingCapitalAccounts.filter(
      (account) =>
        account.relation &&
        account.relation.subType === RELATION_SUB_TYPES.WC_LIABILITY
    );
  }, [workingCapitalAccounts]);

  // ドロップダウン表示用の名前リスト
  const investmentOptions = useMemo(
    () => [...investmentAccounts.map((account) => account.accountName), null],
    [investmentAccounts]
  );

  const depreciationOptions = useMemo(
    () => [...depreciationAccounts.map((account) => account.accountName), null],
    [depreciationAccounts]
  );

  const profitOptions = useMemo(
    () => [...profitAccounts.map((account) => account.accountName), null],
    [profitAccounts]
  );
  ...
  // 運転資本（資産）設定テーブルの設定
  const workingCapitalAssetSettings = useMemo(() => {
    if (workingCapitalAssetAccounts.length === 0) return null;

    const data = workingCapitalAssetAccounts.map((account) => [
      account.accountName,
    ]);
  ...
  }, [workingCapitalAssetAccounts]);

  // 運転資本（負債）設定テーブルの設定
  const workingCapitalLiabilitySettings = useMemo(() => {
    if (workingCapitalLiabilityAccounts.length === 0) return null;

    const data = workingCapitalLiabilityAccounts.map((account) => [
      account.accountName,
    ]);
  ...
  }, [workingCapitalLiabilityAccounts]);
  ...
  return (
    <div className="relation-setting-table">
      ...
      {(workingCapitalAssetSettings || workingCapitalLiabilitySettings) && (
        <>
          <h3>運転資本設定</h3>
          <p className="relation-description">運転資本科目を設定します。</p>
          <div className="working-capital-container">
            {workingCapitalAssetSettings && (
              <div className="hot-table-container">
                <HotTable {...workingCapitalAssetSettings} />
              </div>
            )}
            {workingCapitalLiabilitySettings && (
              <div className="hot-table-container">
                <HotTable {...workingCapitalLiabilitySettings} />
              </div>
            )}
          </div>
        </>
      ...
      {!ppeSettings &&
        ...
        !workingCapitalAssetSettings &&
        !workingCapitalLiabilitySettings && (
          <div className="no-relation-message">
